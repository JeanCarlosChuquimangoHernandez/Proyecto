...
class Payview : NestedScrollView, View.OnFocusChangeListener {
    ...
    private fun init(context: Context,attrs: AttributeSet?,defStyleAttr: Int,defStyleRes: Int){
        val layout = LayoutInflater.from(context)
            .inflate(R.layout.registronegocio,null)
    ...
    private fun initViews(){
        ...
        btn_pay.setOnTouchListener(object : View.OnTouchListener {
            override fun onTouch(v: View?, event: MotionEvent?): Boolean {
                if (event?.action == MotionEvent.ACTION_UP) {
                    // Verifica que todos los campos estén llenos
                    isFillAllComponents = checkAllFormFields()
                    if (isFillAllComponents) {
                        // Obtener datos del carrito y total
                        val cartProducts = Cart.getProducts()
                        val totalPrice = cartProducts.sumOf { it.price * it.quantity }

                        if (cartProducts.isNotEmpty()) {
                            // Guardar en Firebase Firestore
                            guardarDatosEnBaseDeDatos(context, cartProducts, totalPrice)



                            // Enviar notificación
                            val contex = context
                            enviarNotificacionPagoRealizado(contex, totalPrice)

                            // Limpiar el carrito
                            Cart.clearCart()

                            // Mostrar mensaje de confirmación
                            Toast.makeText(v?.context, "Pago realizado exitosamente. El carrito ha sido vaciado.", Toast.LENGTH_LONG).show()

                            // Finalizar actividad o redirigir
                            (v?.context as? Activity)?.finish()
                        } else {
                            Toast.makeText(v?.context, "El carrito está vacío. No se puede procesar el pago.", Toast.LENGTH_SHORT).show()
                        }
                    } else {
                        Toast.makeText(v?.context, "Por favor, completa todos los campos.", Toast.LENGTH_SHORT).show()
                    }
                }
                return false
            }
    ...
    private fun guardarDatosEnBaseDeDatos(context: Context?, cartProducts: List<Product>, totalPrice: Double) {
        if (context == null) {
            Log.e("PayView", "El contexto es nulo, no se puede proceder")
            return
        }

        val db = FirebaseFirestore.getInstance()
        val userId = FirebaseAuth.getInstance().currentUser?.uid ?: "UsuarioDesconocido"

        val data = hashMapOf(
            "userId" to userId,
            "date" to System.currentTimeMillis(),
            "totalPrice" to totalPrice,
            "products" to cartProducts.map { product ->
                mapOf(
        ...
        db.collection("orders")
            ...
            .addOnSuccessListener { documentReference ->
                Log.d("PayView", "Pedido guardado con ID: ${documentReference.id}")
            ...
            .addOnFailureListener { e ->
                Log.e("PayView", "Error al guardar el pedido", e)
                Toast.makeText(context, "Error al registrar el pago", Toast.LENGTH_SHORT).show()
            }
    }
    ...
    private fun enviarNotificacionPagoRealizado(context: Context, totalPrice: Double) {
        val notificationManager =
            context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager

        // Crear el canal de notificación (Android Oreo y superior)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                "payment_channel", // ID del canal
                "Pagos Realizados", // Nombre del canal
                NotificationManager.IMPORTANCE_HIGH
            ).apply {
                description = "Notificaciones de pagos completados"
            }
        ...
        val notification = NotificationCompat.Builder(context, "payment_channel")
            .setSmallIcon(R.drawable.ic_card) // Cambia esto por un ícono válido en tu proyecto
            .setContentTitle("Pago Completado")
            .setContentText("Gracias por tu compra. Total pagado: S/ %.2f".format(totalPrice))
